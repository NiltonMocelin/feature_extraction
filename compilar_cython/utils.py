...
def contador_flags(lista_flags:list, flag = None):
    contador =0
    if flag:
...
def calcular_mean(lista_valores):
    ...
    for val in lista_valores:
        soma += val
    retorno = round(soma/len(lista_valores), PRECISAO)
...
def calcular_median(lista_valores):
    ...
    return round((lista_ordenada[int(meio)] + lista_ordenada[int(meio + 1)])/2, PRECISAO)
