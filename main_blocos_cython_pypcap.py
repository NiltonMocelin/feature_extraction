...
def format_csv_line(valores, num_string_fields=7):
    partes = []
    for i, val in enumerate(valores):
        val_str = str(val)
        if i < num_string_fields:
...
def _process_bloco_worker(job):
    return process_bloco_pypcap(job)
...
def gerar_blocos_processar_otimizado2(host_a, host_b,host_a_port, host_b_port, lista_ts_raw_pkts, file_path, block_size, idle_timeout, linktype, tabela_db, proto, service_class, 
    ...
    lista_blocos = list(dividir_e_filtrar_rajadas(lista_ts_raw_pkts, block_size, idle_timeout))

    if not lista_blocos:
        print(f'sem blocos validos')
        return

    jobs = [{'parametros': parametros, 'id_bloco': b['id'], 'packets': b['packets']}
            for b in lista_blocos]

    with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
        resultados_brutos = pool.map(_process_bloco_worker, jobs)
    ...
    header_cols = resultados_brutos[0][0]
    header_filename = f"headers_{tabela_db}.csv"
    escreverArquivo(tabela_db, header_filename, ','.join(header_cols), append=False)

    filename_out = f"{tabela_db}_{app_class}_{pid}.csv"
    print(f"Escrevendo {len(resultados_brutos)} resultados em {filename_out}")
