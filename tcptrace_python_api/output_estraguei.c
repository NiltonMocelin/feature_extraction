...
PrintTrace(
    tcp_pair *ptp)
{
    // Nilton Temos que editar essa funcao...
    // char bufff[1000];
    // size_t offset = 0;
    //Nilton
    ...
    char outroBuffer[100]; // Limpa o buffer antes de usá-lo
   ...
   if(csv || tsv || (sv != NULL)) {
       fprintf(stdout,"%s%s%s%s%s%s%s%s",
        ...
        //escrever_resultados_str("a_hostname", ptp->a_hostname);
        //escrever_resultados_str("b_hostname", ptp->b_hostname);
        //escrever_resultados_str("a_portname", ptp->a_portname);
        //escrever_resultados_str("b_portname", ptp->b_portname);
	...
	*/ 
       ...
       //escrever_resultados_str("first_time", outroBuffer);
       snprintf(outroBuffer, sizeof(outroBuffer), "%ld.%ld", (long)ptp->last_time.tv_sec, (long)ptp->last_time.tv_usec);
    ...
    udp_pair *pup)
{
    ...
    //escrever_resultados_str("host_a", host1);
    fprintf(stdout,"\thost %-4s      %s\n",
	    (snprintf(bufl,sizeof(bufl),"%s:", host2),bufl), pup->b_endpoint);
    //escrever_resultados_str("host_b", host2);
    fprintf(stdout,"\n");

    fprintf(stdout,"\tfirst packet:  %s\n", ts2ascii(&pup->first_time));
    //escrever_resultados_str("first_packet_time", ts2ascii(&pup->first_time));
    fprintf(stdout,"\tlast packet:   %s\n", ts2ascii(&pup->last_time));
    //escrever_resultados_str("last_packet_time", ts2ascii(&pup->last_time));
    ...
    fprintf(stdout,"\ttotal packets: %" FS_ULL "\n", pup->packets);
    //escrever_resultados_int("total_packets", pup->packets);

    fprintf(stdout,"\tfilename:      %s\n", pup->filename);
    //escrever_resultados_str("filename", pup->filename);
    ...
    //escrever_resultados_int("a2b_data_bytes_sent", pab->data_bytes);
    //escrever_resultados_int("b2a_data_bytes_sent", pba->data_bytes);
    //escrever_resultados_int("total_data_bytes_sent", pab->data_bytes + pba->data_bytes);
    ...
    //escrever_resultados_float("a2b_throughput_Bps", (double) (pab->data_bytes) / etime);
    //escrever_resultados_float("b2a_throughput_Bps", (double) (pba->data_bytes) / etime);
    }
}
