...
def get_filtros(file_name):
    ...
    sniffer = pcap.pcap(name=file_name, timeout_ms=0)
    link_type = sniffer.datalink()

    # for pkt in leitor(file_name):
    for ts, pkt in sniffer:
        pkt_dict = montar_pkt_to_dict(pkt, link_type)
        if pkt_dict == None:
            continue

        # if(not pkt_dict.get('IP')): # and (pkt.haslayer(UDP) == False or pkt.haslayer(TCP) == False)) :
        #     continue
        # proto = pkt_dict['ipv4']['transport_proto']
        proto=pkt_dict['ipv4']['transport_proto']

        if pkt_dict.get(proto) == None:
            print(f"no proto {proto} in pkt_dict: {pkt_dict}")
            continue

        ip_src = pkt_dict['ipv4']['src_ip']
        ip_dst = pkt_dict['ipv4']['dst_ip']
        sport = pkt_dict[proto]['src_port'] #ja é int
        dport = pkt_dict[proto]['dst_port'] #ja é int
        ...
        filtro = f'host {ip_src} and host {ip_dst} and {aux_proto} port {sport} and {aux_proto} port {dport}'
        # filtro = f'(src host {ip_src} and dst host {ip_dst} and {aux_proto} src port {sport} and {aux_proto} dst port {dport}) or (src host {ip_dst} and dst host {ip_src} and {aux_proto} src port {dport} and {aux_proto} dst port {sport})'
...
def pypcap_apply_filter_to_file(input_file, filter_string, output_file):
    ...
    sniffer = pcap.pcap(name=input_file, timeout_ms=0)
    link_type = sniffer.datalink()
    print(f"Applying filter: {filter_string} to {input_file}, saving to {output_file}")
    sniffer.setfilter(filter_string)
...
def process_pcap(file_name):
    ...
    for key in dict_fluxos:
        ...
        pypcap_apply_filter_to_file(file_name, key, caminho_saida+dict_fluxos[key])
