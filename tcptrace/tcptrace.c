...
//  if (run_continuously) {
    ...
    /* check for parallel flag (-j for number of threads) */
    for (i = 1; i < argc; i++) {
...
#ifdef _OPENMP
    if (num_threads > 1 && numfiles > 1) {
        printf("Processing %d files with %d parallel workers (OpenMP)\n", numfiles, num_threads);
...
#pragma omp parallel for schedule(dynamic)
    ...
    if (num_threads > 1 && numfiles > 1) {
        ...
        int current_file = 0;
        int *pipes = calloc(num_files * 2, sizeof(int));

        printf("Processing %d files with %d parallel workers (fork)\n", numfiles, num_threads);

        while (current_file < num_files || active_children > 0) {
            while (current_file < num_files && active_children < num_threads) {
                int pid = fork();
                if (pid == 0) {
                    ProcessFile(filenames[current_file]);
                    _exit(0);
                } else if (pid > 0) {
                    active_children++;
                    current_file++;
...
trace_context *
tcptrace_create_context(void)
{
    trace_context *ctx;

    ctx = (trace_context *) MallocZ(sizeof(trace_context));
    if (!ctx)
        return NULL;

    trace_init_context_full(ctx, 64);
    udptrace_init_context(ctx);
...
void
tcptrace_destroy_context(trace_context *ctx)
{
    if (!ctx)
        return;

    trace_done_context(ctx);
    udptrace_done_context(ctx);
...
static void
ProcessFileWithContext(trace_context *ctx, char *filename)
{
    char *old_cur_filename = cur_filename;
    u_long old_pnum = pnum;
    u_long old_tcp_trace_count = tcp_trace_count;
    u_long old_udp_trace_count = udp_trace_count;
    u_long old_bad_ip_checksums = bad_ip_checksums;
    u_long old_bad_tcp_checksums = bad_tcp_checksums;
    u_long old_bad_udp_checksums = bad_udp_checksums;
    struct timeval old_first_packet = first_packet;
    struct timeval old_last_packet = last_packet;

    cur_filename = filename;

    ProcessFile(filename);

    ctx->packet_count = pnum - old_pnum;
    ctx->tcp_count = tcp_trace_count - old_tcp_trace_count;
    ctx->udp_count = udp_trace_count - old_udp_trace_count;
    ctx->bad_ip_checksums = bad_ip_checksums - old_bad_ip_checksums;
    ctx->bad_tcp_checksums = bad_tcp_checksums - old_bad_tcp_checksums;
    ctx->bad_udp_checksums = bad_udp_checksums - old_bad_udp_checksums;

    if (ZERO_TIME(&first_packet))
        ctx->first_packet = old_first_packet;
    else
        ctx->first_packet = first_packet;

    ctx->last_packet = last_packet;

    cur_filename = old_cur_filename;
    pnum = old_pnum;
    tcp_trace_count = old_tcp_trace_count;
    udp_trace_count = old_udp_trace_count;
    bad_ip_checksums = old_bad_ip_checksums;
    bad_tcp_checksums = old_bad_tcp_checksums;
    bad_udp_checksums = old_bad_udp_checksums;
    first_packet = old_first_packet;
    last_packet = old_last_packet;
}


int
tcptrace_analyze_file(trace_context *ctx, const char *filename, int num_threads)
{
    int i;
    int ret = 0;
    char *single_file;
    ...
    if (num_threads <= 1) {
        ProcessFileWithContext(ctx, (char *)filename);

        ctx->packet_count = pnum;
        ctx->tcp_count = tcp_trace_count;
        ctx->udp_count = udp_trace_count;
        ctx->first_packet = first_packet;
        ctx->last_packet = last_packet;
        ctx->trace_duration = elapsed(first_packet, last_packet);
        ctx->bad_ip_checksums = bad_ip_checksums;
        ctx->bad_tcp_checksums = bad_tcp_checksums;
        ctx->bad_udp_checksums = bad_udp_checksums;
        ctx->num_tcp_pairs = num_tcp_pairs;
        ctx->num_udp_pairs = num_udp_pairs;

        if (do_udp) {
            extract_udp_connections(ctx);
        }
        extract_tcp_connections(ctx);

        return 0;
    }

#ifdef _OPENMP
    single_file = strdup(filename);
    if (!single_file)
        return -1;
...
#pragma omp parallel
    {
        ...
        int num_files_total = 1;
        int chunk_size = 1;
        int start_idx = tid * chunk_size;
        int end_idx = start_idx + chunk_size;

        if (start_idx < num_files_total && tid < num_threads) {
            int j;
            for (j = start_idx; j < end_idx && j < num_files_total; j++) {
                ProcessFileWithContext(ctx, single_file);
            }
        }
    }
    ...
    int linktype)
{
    ...
    void *plast;
    int phystype;
    struct timeval first_packet_local;
    struct timeval last_packet_local;
    int tcp_count_local = 0;
    int udp_count_local = 0;
    int packet_count_local = 0;
    unsigned char packet_with_dummy[14 + 65535];

    if (!ctx || !timestamps || !captured_lens || !packets || num_packets <= 0) {
        return -1;
    }

    /* Reset context for fresh batch analysis */
    /* This ensures clean state for each batch call */
    /* First, clean up any leftover data from previous batch calls */
    if (ctx->tcp_conns) {
        int ix;
    ...
    /* Set batch context for trace.c functions to use context storage */
    set_batch_context(ctx);

    /* Reset global state for fresh batch analysis */
    trace_reset_globals();

    ZERO_TIME(&first_packet_local);
    ZERO_TIME(&last_packet_local);

    for (i = 0; i < num_packets; i++) {
        struct timeval current_time = timestamps[i];
        int caplen = captured_lens[i];
        ...
        int ip_offset = 14;  /* Default: Ethernet */
        if (linktype == 12) {
            ip_offset = 0;    /* Raw IP (DLT_RAW): no link-layer header */
        } else if (linktype == 113) {
            ip_offset = 16;   /* Linux SLL (DLT_LINUX_SLL): 16-byte header */
        }

        /* For Ethernet: check for non-IP frames and wrap with dummy Ethernet header */
        if (linktype == 1) {
            unsigned short ethertype = (pkt[12] << 8) | pkt[13];
            if (ethertype != ETHERTYPE_IP && ethertype != ETHERTYPE_IPV6) {
                memset(packet_with_dummy, 0, 12);
                packet_with_dummy[12] = 0x08;
                packet_with_dummy[13] = 0x00;
                memcpy(packet_with_dummy + 14, pkt, caplen);
                pkt = packet_with_dummy;
                phys = pkt;
                caplen += 14;
                ip_offset = 14;
            }
        ...
        /* For non-Ethernet link types with a link-layer header: create dummy
         * Ethernet header and place the IP data after it */
        if (linktype != 1 && ip_offset > 0) {
            if (caplen <= ip_offset) {
                continue;
            }
            memset(packet_with_dummy, 0, 12);
            packet_with_dummy[12] = 0x08;
            packet_with_dummy[13] = 0x00;
            memcpy(packet_with_dummy + 14, pkt + ip_offset, caplen - ip_offset);
            pkt = packet_with_dummy;
            phys = pkt;
            caplen = 14 + (caplen - ip_offset);
            ip_offset = 14;
        }

        pip = (struct ip *)(pkt + ip_offset);
        plast = pkt + caplen - 1;

        if (ZERO_TIME(&first_packet_local)) {
            first_packet_local = current_time;
        }
        last_packet_local = current_time;
        ...
        ret = gettcp(pip, &ptcp, &plast);
        if (ret == 0) {
            tcp_pair *ptp = dotrace(pip, ptcp, plast);
            if (ptp && !ptp->ignore_pair) {
                tcp_count_local++;
            }
        } else if (ret < 0) {
            ret = getudp(pip, &pudp, &plast);
            if (do_udp && ret == 0) {
                udp_pair *pup = udpdotrace(pip, pudp, plast);
                if (pup && !pup->ignore_pair) {
                    udp_count_local++;
                }
            }
        }
    ...
    /* Clear batch context - trace.c will now use fresh storage on next call */
    set_batch_context(NULL);

    ctx->packet_count = packet_count_local;
    ctx->tcp_count = tcp_count_local;
    ctx->udp_count = udp_count_local;
    ctx->first_packet = first_packet_local;
    ctx->last_packet = last_packet_local;
    ctx->trace_duration = elapsed(first_packet_local, last_packet_local);
...
char *buffer_colunas;
char *buffer_resultados;
int offset_buffer_colunas = 0;
int offset_buffer_resultados = 0;

void inicializar_memoria_compartilhada() {
    // Mapeia memória compartilhada anônima (não precisa de arquivo)
    buffer_colunas = mmap(NULL, BUFFER_SIZE, PROT_READ | PROT_WRITE, 
                          MAP_SHARED | MAP_ANONYMOUS, -1, 0);
    buffer_resultados = mmap(NULL, BUFFER_SIZE, PROT_READ | PROT_WRITE, 
                             MAP_SHARED | MAP_ANONYMOUS, -1, 0);
}

void destruir_memoria_compartilhada() {
    if (buffer_colunas != NULL) {
        munmap(buffer_colunas, BUFFER_SIZE);
        munmap(buffer_resultados, BUFFER_SIZE);
        // munmap(offset_buffer_colunas, sizeof(int));
        // munmap(offset_buffer_resultados, sizeof(int));
        
        buffer_colunas = NULL;
        buffer_resultados = NULL;
        // offset_buffer_colunas = NULL;
        // offset_buffer_resultados = NULL;
    }
}


// Isso rodará automaticamente quando a DLL/SO for carregada
void __attribute__ ((constructor)) setup_shared_mem() {
    if (buffer_colunas == NULL) {
        inicializar_memoria_compartilhada();
    }
}

// Executa automaticamente quando o programa/script Python fechar
void __attribute__ ((destructor)) cleanup_shared_mem() {
    destruir_memoria_compartilhada();
}

void escrever_resultados_str(char *coluna, char *valor)
{ //o que esta acontecendo aqui

    offset_buffer_colunas += snprintf(buffer_colunas + offset_buffer_colunas, BUFFER_SIZE - offset_buffer_colunas, "%s,", coluna);
    offset_buffer_resultados += snprintf(buffer_resultados + offset_buffer_resultados, BUFFER_SIZE - offset_buffer_resultados, "%s,", valor);
...
{ //o que esta acontecendo aqui
    ...
    offset_buffer_colunas += snprintf(buffer_colunas + offset_buffer_colunas, BUFFER_SIZE - offset_buffer_colunas, "%s,", coluna);
    offset_buffer_resultados += snprintf(buffer_resultados + offset_buffer_resultados, BUFFER_SIZE - offset_buffer_resultados, "%ld,", valor);
...
{ //o que esta acontecendo aqui
    ...
    offset_buffer_colunas += snprintf(buffer_colunas + offset_buffer_colunas, BUFFER_SIZE - offset_buffer_colunas, "%s,", coluna);
    offset_buffer_resultados += snprintf(buffer_resultados + offset_buffer_resultados, BUFFER_SIZE - offset_buffer_resultados, "%f,", valor);
    
    return;
}

void reset_buffers(){
	memset(buffer_colunas, 0, BUFFER_SIZE);
	memset(buffer_resultados, 0, BUFFER_SIZE);
	offset_buffer_colunas = 0;
    offset_buffer_resultados = 0;
}
    ...
    char *argv)
{
	...
	//  if (run_continuously) {
		...
		// close(original_stdout);
		// //
		msync(buffer_colunas, BUFFER_SIZE, MS_SYNC);
    	msync(buffer_resultados, BUFFER_SIZE, MS_SYNC);
	    exit(0);
		...
		// 1. Verifique se o filho não deu SegFault
        if (WIFSIGNALED(status)) {
            return "erro;filho_morreu_sinal";
        }

        // 2. Tente ler os dados. Se buffer_colunas[0] for '\0', 
        // significa que o Pai ainda não "viu" a escrita do filho.
        if (buffer_colunas[0] == '\0') {
             // Pequeno fôlego para o kernel sincronizar as páginas (raro, mas acontece)
             usleep(1000); 
        }


		static char buffer[BUFFER_SIZE*2]; //https://stackoverflow.com/questions/14416759/return-char-string-from-a-function#:~:text=If%20you%20want%20to%20return,2%20Comments
		...
		// 4. LOG DE DEBUG: Se imprimir vazio aqui, o mmap falhou
        if (buffer[0] == ';' || buffer[0] == '\0') {
            fprintf(stderr, "[DEBUG PAI] Alerta: Buffer capturado vazio! %s\n", argumentos[6]);
        }
    ...
    char *argv[])
{
	...
	//Nilton -fiz outras mas essa eh a mais importante - redirecionar stdout para o buffer
	// Opcao 1...
	// for(int i =0; i <argc; i++)
	// printf("Argv[i]: %s", argv[i]);

	static char buffer[1024*64+1]; //https://stackoverflow.com/questions/14416759/return-char-string-from-a-function#:~:text=If%20you%20want%20to%20return,2%20Comments
	memset(buffer, 0, sizeof(buffer)); //limpar buffer antes de usar
	// 2. Limpeza preventiva de qualquer resquício global
    fflush(stdout);
    setvbuf(stdout, NULL, _IONBF, 0); // Desativa o buffering do stdout (modo direto)

	// 1. Salva o STDOUT original
    int saved_stdout = dup(STDOUT_FILENO);

	// 2. Abre um arquivo temporário real (evita o limite de 64KB do pipe)
    FILE *tmp = tmpfile(); 
    if (!tmp) return "Error: tmpfile";
    int tmp_fd = fileno(tmp);

    // 3. Redireciona STDOUT para o arquivo
    // dup2(tmp_fd, STDOUT_FILENO);
    dup2(tmp_fd, STDOUT_FILENO);

	// after print, should call fflush(stdout); // Flush the buffer to ensure data goes into the pipe

	// Opcao 2 -redirecionar stdout para file.. (mais seguro - se nao funcionar a primeira)
...
//	udptrace_init();
  ...
  //  if (run_continuously) {
	...
	//Nilton opcao1
	// Lê do pipe para o buffer
	
	fflush(stdout); // 4. Flush é essencial para garantir que o dado foi para o pipe
	fsync(tmp_fd);

	// 4. Restaura o STDOUT imediatamente
    dup2(saved_stdout, STDOUT_FILENO);
    close(saved_stdout);
	// 6. Volta para o modo de buffering padrão (opcional)
    setvbuf(stdout, NULL, _IOLBF, BUFSIZ);

    // 5. Volta para o início do arquivo temporário e lê
    rewind(tmp);
	// Lendo o conteúdo total do arquivo para o buffer
    // fread é melhor que fgets aqui para pegar múltiplas linhas de uma vez
    size_t n = fread(buffer, 1, 1024 * 64, tmp);
    buffer[n] = '\0'; // Garante o fim da string

	// 7. Limpa novamente para garantir que nada fique para a próxima chamada
    fflush(NULL);
 	// 6. Fecha o arquivo (o SO deleta o tmpfile automaticamente aqui)
    fclose(tmp); 
	// printf("Read from buffer: %s\n", buffer);
	//Nilton opcao1
	// static char cc[]= "ola"; return cc; // funciona
...
>>>>>>> 86f18337963adc76f3ea0534400b0dd84b3447de:tcptrace_python_api/tcptrace.c
