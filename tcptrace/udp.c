...
void
udptrace_init_context(trace_context *ctx)
{
    if (!ctx)
        return;
    ...
    ctx->num_udp_pairs = 0;
    ctx->utp = NULL;
    ctx->udp_snap_root = NULL;
    ctx->udp_count = 0;
    ctx->bad_udp_checksums = 0;

    ctx->utp = (udp_pair **) MallocZ(max_udp_pairs * sizeof(udp_pair *));
    ctx->num_udp_pairs = 0;
}


void
udptrace_done_context(trace_context *ctx)
{
    int ix;
    ...
    if (ctx->utp) {
        for (ix = 0; ix < ctx->num_udp_pairs; ++ix) {
            if (ctx->utp[ix]) {
                FreeUdpPair(ctx->utp[ix]);
            }
        }
        free(ctx->utp);
        ctx->utp = NULL;
    ...
    if (ctx->udp_snap_root) {
        FreeUdpPair(ctx->udp_snap_root);
        ctx->udp_snap_root = NULL;
    }
...
void
extract_udp_connections(trace_context *ctx)
{
    int i, count;
    udp_conn_info *conns;
    udp_pair *pup;
    ...
    for (i = 0, count = 0; i < ctx->num_udp_pairs; ++i) {
        pup = ctx->utp[i];
    ...
    conns = (udp_conn_info *) MallocZ(count * sizeof(udp_conn_info));
    ctx->udp_conns = conns;
    ctx->num_udp_conns = count;
    ...
    for (i = 0, count = 0; i < ctx->num_udp_pairs; ++i) {
        ...
        conns[count].a_endpoint = pup->a_endpoint ? strdup(pup->a_endpoint) : strdup("");
        conns[count].b_endpoint = pup->b_endpoint ? strdup(pup->b_endpoint) : strdup("");
        conns[count].a_hostname = pup->a_hostname ? strdup(pup->a_hostname) : strdup("");
        conns[count].b_hostname = pup->b_hostname ? strdup(pup->b_hostname) : strdup("");
        conns[count].a_portname = pup->a_portname ? strdup(pup->a_portname) : strdup("");
        conns[count].b_portname = pup->b_portname ? strdup(pup->b_portname) : strdup("");

        conns[count].first_time = pup->first_time;
        conns[count].last_time = pup->last_time;
        conns[count].elapsed_time = elapsed(pup->first_time, pup->last_time);

        conns[count].a2b_packets = pab->packets;
        conns[count].a2b_data_bytes = pab->data_bytes;
        conns[count].a2b_min_dg_size = pab->min_dg_size;
        conns[count].a2b_max_dg_size = pab->max_dg_size;

        if (conns[count].elapsed_time > 0)
            conns[count].a2b_throughput = (double)pab->data_bytes / (conns[count].elapsed_time / 1000000.0);
        else
            conns[count].a2b_throughput = 0.0;

        conns[count].b2a_packets = pba->packets;
        conns[count].b2a_data_bytes = pba->data_bytes;
        conns[count].b2a_min_dg_size = pba->min_dg_size;
        conns[count].b2a_max_dg_size = pba->max_dg_size;

        if (conns[count].elapsed_time > 0)
            conns[count].b2a_throughput = (double)pba->data_bytes / (conns[count].elapsed_time / 1000000.0);
        else
            conns[count].b2a_throughput = 0.0;

        conns[count].filename = pup->filename ? strdup(pup->filename) : strdup("");
...
>>>>>>> 86f18337963adc76f3ea0534400b0dd84b3447de:tcptrace_python_api/udp.c
