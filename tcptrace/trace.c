...
static trace_context *current_context = NULL;
static struct ptp_snap *tcp_snap_root = NULL;
...
/* batch processing context - for thread-safe batch analysis */
static trace_context *current_batch_ctx = NULL;
...
/* Set the current batch context for thread-safe processing */
void set_batch_context(trace_context *ctx) {
    current_batch_ctx = ctx;
}

/* Get the current batch context */
trace_context* get_batch_context(void) {
    return current_batch_ctx;
}

/* Reset trace globals for fresh batch processing */
void trace_reset_globals(void) {
    num_tcp_pairs = 0;
    tcp_trace_count = 0;
    clear_hashtable();
}
    ...
    struct tcphdr *ptcp)
{
    ...
    /* Check if we're in batch mode with context */
    trace_context *batch_ctx = get_batch_context();
    int in_batch_mode = (batch_ctx != NULL);
    ...
    if (in_batch_mode) {
        /* Use context-based storage for batch processing */
        int idx = batch_ctx->num_tcp_pairs;  /* Current count = next index */
        ++batch_ctx->num_tcp_pairs;
        if ((batch_ctx->num_tcp_pairs) >= batch_ctx->max_tcp_pairs) {
            /* Expand context tcp_pair array if needed */
            int new_max = batch_ctx->max_tcp_pairs + 16;
            tcp_pair **new_ttp = (tcp_pair **)realloc(batch_ctx->ttp, new_max * sizeof(tcp_pair *));
            if (new_ttp) {
                batch_ctx->ttp = new_ttp;
                batch_ctx->max_tcp_pairs = new_max;
            }
        }
        batch_ctx->ttp[idx] = ptp;
        ptp->ignore_pair = FALSE;  /* Default to not ignored in batch mode */
    } else {
...
/* Clear the hash table for fresh batch processing */
static void clear_hashtable(void) {
    int i;
    for (i = 0; i < HASH_TABLE_SIZE; i++) {
        ptp_hashtable[i] = NULL;
    }
}
...
void
trace_init_context(trace_context *ctx)
{
    if (!ctx)
        return;
    ...
    ctx->result = 0;
    ctx->error_msg = NULL;
    ctx->max_tcp_pairs = 64;
    ctx->bad_ip_checksums = 0;
    ctx->bad_tcp_checksums = 0;
    ctx->ignore_pairs = NULL;
    ctx->more_conns_ignored = FALSE;

    if (ctx->max_tcp_pairs <= 0)
        ctx->max_tcp_pairs = 64;

    ctx->ttp = (tcp_pair **) MallocZ(ctx->max_tcp_pairs * sizeof(tcp_pair *));
    ctx->ignore_pairs = (Bool *) MallocZ(ctx->max_tcp_pairs * sizeof(Bool));
    ctx->num_tcp_pairs = 0;
}


void
trace_init_context_full(trace_context *ctx, int max_tcp_pairs)
{
    if (!ctx)
        return;

    ctx->max_tcp_pairs = max_tcp_pairs > 0 ? max_tcp_pairs : 64;
    trace_init_context(ctx);
}


void
trace_done_context(trace_context *ctx)
{
    int ix;
    ...
    if (ctx->ttp) {
        for (ix = 0; ix < ctx->num_tcp_pairs; ++ix) {
            if (ctx->ttp[ix]) {
                FreeTcpPair(ctx->ttp[ix]);
            }
        }
        free(ctx->ttp);
        ctx->ttp = NULL;
    ...
    if (ctx->tcp_snap_root) {
        FreePtpSnap(ctx->tcp_snap_root);
        ctx->tcp_snap_root = NULL;
    }

    if (ctx->ignore_pairs) {
        free(ctx->ignore_pairs);
        ctx->ignore_pairs = NULL;
    }

    if (ctx->tcp_conns) {
        int ix;
        for (ix = 0; ix < ctx->num_tcp_conns; ++ix) {
            tcp_conn_info *ci = &ctx->tcp_conns[ix];
            if (ci->a_endpoint) free(ci->a_endpoint);
            if (ci->b_endpoint) free(ci->b_endpoint);
            if (ci->a_hostname) free(ci->a_hostname);
            if (ci->b_hostname) free(ci->b_hostname);
            if (ci->a_portname) free(ci->a_portname);
            if (ci->b_portname) free(ci->b_portname);
            if (ci->host_letter_a) free(ci->host_letter_a);
            if (ci->host_letter_b) free(ci->host_letter_b);
    ...
    if (ctx->udp_conns) {
        int ix;
        for (ix = 0; ix < ctx->num_udp_conns; ++ix) {
            udp_conn_info *ci = &ctx->udp_conns[ix];
            if (ci->a_endpoint) free(ci->a_endpoint);
            if (ci->b_endpoint) free(ci->b_endpoint);
            if (ci->a_hostname) free(ci->a_hostname);
            if (ci->b_hostname) free(ci->b_hostname);
...
void
extract_tcp_connections(trace_context *ctx)
{
    int i, count;
    tcp_conn_info *conns;
    tcp_pair *ptp;
    tcb *pab, *pba;
    u_long pab_last, pba_last;
    u_llong stream_length_pab, stream_length_pba;

    if (!ctx)
        return;

    for (i = 0, count = 0; i < ctx->num_tcp_pairs; ++i) {
        ptp = ctx->ttp[i];
        if (ptp && !ptp->ignore_pair && !ptp->inactive)
            count++;
    }
    ...
    conns = (tcp_conn_info *) MallocZ(count * sizeof(tcp_conn_info));
    ctx->tcp_conns = conns;
    ctx->num_tcp_conns = count;
    ...
    for (i = 0, count = 0; i < ctx->num_tcp_pairs; ++i) {
        ...
        conns[count].a_endpoint = ptp->a_endpoint ? strdup(ptp->a_endpoint) : strdup("");
        conns[count].b_endpoint = ptp->b_endpoint ? strdup(ptp->b_endpoint) : strdup("");
        conns[count].a_hostname = ptp->a_hostname ? strdup(ptp->a_hostname) : strdup("");
        conns[count].b_hostname = ptp->b_hostname ? strdup(ptp->b_hostname) : strdup("");
        conns[count].a_portname = ptp->a_portname ? strdup(ptp->a_portname) : strdup("");
        conns[count].b_portname = ptp->b_portname ? strdup(ptp->b_portname) : strdup("");
        conns[count].host_letter_a = pab->host_letter ? strdup(pab->host_letter) : strdup("");
        conns[count].host_letter_b = pba->host_letter ? strdup(pba->host_letter) : strdup("");

        conns[count].first_time = ptp->first_time;
        conns[count].last_time = ptp->last_time;
        conns[count].elapsed_time = elapsed(ptp->first_time, ptp->last_time);

        if (pab->reset_count > 0 || pba->reset_count > 0) {
            conns[count].is_reset = 1;
            conns[count].complete = 0;
        } else if (pab->syn_count >= 1 && pba->syn_count >= 1 &&
                   pab->fin_count >= 1 && pba->fin_count >= 1) {
            conns[count].is_reset = 0;
            conns[count].complete = 1;
        } else {
            conns[count].is_reset = 0;
            conns[count].complete = 0;
        }

        conns[count].a2b_packets = pab->packets;
        conns[count].a2b_data_bytes = pab->data_bytes;
        conns[count].a2b_data_pkts = pab->data_pkts;
        conns[count].a2b_rexmit_bytes = pab->rexmit_bytes;
        conns[count].a2b_rexmit_pkts = pab->rexmit_pkts;
        conns[count].a2b_unique_bytes = pab->unique_bytes;
        conns[count].a2b_ack_pkts = pab->ack_pkts;
        conns[count].a2b_pure_ack_pkts = pab->pureack_pkts;
        conns[count].a2b_sack_pkts = pab->num_sacks;
        conns[count].a2b_dsack_pkts = pab->num_dsacks;
        conns[count].a2b_mss = pab->mss;
        conns[count].a2b_max_seg_size = pab->max_seg_size;
        conns[count].a2b_min_seg_size = pab->min_seg_size;
        conns[count].a2b_win_max = pab->win_max;
        conns[count].a2b_win_min = pab->win_min;
        conns[count].a2b_win_zero_ct = pab->win_zero_ct;
        conns[count].a2b_win_tot = pab->win_tot;
        conns[count].a2b_out_order_pkts = pab->out_order_pkts;
        conns[count].a2b_zwnd_probes = pab->num_zwnd_probes;
        conns[count].a2b_zwnd_probe_bytes = pab->zwnd_probe_bytes;
        conns[count].a2b_urg_data_pkts = pab->urg_data_pkts;
        conns[count].a2b_urg_data_bytes = pab->urg_data_bytes;
        conns[count].a2b_trunc_bytes = pab->trunc_bytes;
        conns[count].a2b_trunc_segs = pab->trunc_segs;
        conns[count].a2b_num_sacks = pab->num_sacks;
        conns[count].a2b_max_sack_blocks = pab->max_sack_blocks;
        conns[count].a2b_num_hardware_dups = pab->num_hardware_dups;
        conns[count].a2b_syn_count = pab->syn_count;
        conns[count].a2b_fin_count = pab->fin_count;
        conns[count].a2b_reset_count = pab->reset_count;
        conns[count].a2b_sacks_sent = pab->sacks_sent;

        conns[count].a2b_rtt_count = pab->rtt_count;
        conns[count].a2b_rtt_min = pab->rtt_min;
        conns[count].a2b_rtt_max = pab->rtt_max;
        conns[count].a2b_rtt_sum = pab->rtt_sum;
        conns[count].a2b_rtt_sum2 = pab->rtt_sum2;
        conns[count].a2b_rtt_dupack = pab->rtt_dupack;
        conns[count].a2b_rtt_triple_dupack = pab->rtt_triple_dupack;
        conns[count].a2b_rtt_amback = pab->rtt_amback;
        conns[count].a2b_rtt_cumack = pab->rtt_cumack;
        conns[count].a2b_rtt_nosample = pab->rtt_nosample;
        conns[count].a2b_retr_max = pab->retr_max;
        conns[count].a2b_retr_min_tm = pab->retr_min_tm;
        conns[count].a2b_retr_max_tm = pab->retr_max_tm;
        conns[count].a2b_retr_tm_sum = pab->retr_tm_sum;
        conns[count].a2b_retr_tm_sum2 = pab->retr_tm_sum2;

        conns[count].a2b_initialwin_bytes = pab->initialwin_bytes;
        conns[count].a2b_initialwin_segs = pab->initialwin_segs;
        conns[count].a2b_idle_max = pab->idle_max;

        pab_last = (pab->reset_count > 0) ? pab->latest_seq : pab->fin;
        if (pab->syn_count > 0 && pab->fin_count > 0) {
            if (pab_last > pab->syn)
                stream_length_pab = (u_llong)pab_last - pab->syn - 1;
            else
                stream_length_pab = 0;
        } else {
            stream_length_pab = 0;
        }
        conns[count].a2b_stream_length = stream_length_pab;
        conns[count].a2b_missed_data = (stream_length_pab > pab->unique_bytes) ?
                                       (stream_length_pab - pab->unique_bytes) : 0;

        conns[count].a2b_data_xmit_time = elapsed(pab->first_data_time, pab->last_data_time) / 1000000.0;
        if (conns[count].elapsed_time > 0)
            conns[count].a2b_throughput = (double)pab->unique_bytes / (conns[count].elapsed_time / 1000000.0);
        else
            conns[count].a2b_throughput = 0.0;

        conns[count].a2b_window_scale = pab->window_scale;
        conns[count].a2b_f1323_ws = pab->f1323_ws;
        conns[count].a2b_f1323_ts = pab->f1323_ts;
        conns[count].a2b_fsack_req = pab->fsack_req;

        conns[count].b2a_packets = pba->packets;
        conns[count].b2a_data_bytes = pba->data_bytes;
        conns[count].b2a_data_pkts = pba->data_pkts;
        conns[count].b2a_rexmit_bytes = pba->rexmit_bytes;
        conns[count].b2a_rexmit_pkts = pba->rexmit_pkts;
        conns[count].b2a_unique_bytes = pba->unique_bytes;
        conns[count].b2a_ack_pkts = pba->ack_pkts;
        conns[count].b2a_pure_ack_pkts = pba->pureack_pkts;
        conns[count].b2a_sack_pkts = pba->num_sacks;
        conns[count].b2a_dsack_pkts = pba->num_dsacks;
        conns[count].b2a_mss = pba->mss;
        conns[count].b2a_max_seg_size = pba->max_seg_size;
        conns[count].b2a_min_seg_size = pba->min_seg_size;
        conns[count].b2a_win_max = pba->win_max;
        conns[count].b2a_win_min = pba->win_min;
        conns[count].b2a_win_zero_ct = pba->win_zero_ct;
        conns[count].b2a_win_tot = pba->win_tot;
        conns[count].b2a_out_order_pkts = pba->out_order_pkts;
        conns[count].b2a_zwnd_probes = pba->num_zwnd_probes;
        conns[count].b2a_zwnd_probe_bytes = pba->zwnd_probe_bytes;
        conns[count].b2a_urg_data_pkts = pba->urg_data_pkts;
        conns[count].b2a_urg_data_bytes = pba->urg_data_bytes;
        conns[count].b2a_trunc_bytes = pba->trunc_bytes;
        conns[count].b2a_trunc_segs = pba->trunc_segs;
        conns[count].b2a_num_sacks = pba->num_sacks;
        conns[count].b2a_max_sack_blocks = pba->max_sack_blocks;
        conns[count].b2a_num_hardware_dups = pba->num_hardware_dups;
        conns[count].b2a_syn_count = pba->syn_count;
        conns[count].b2a_fin_count = pba->fin_count;
        conns[count].b2a_reset_count = pba->reset_count;
        conns[count].b2a_sacks_sent = pba->sacks_sent;

        conns[count].b2a_rtt_count = pba->rtt_count;
        conns[count].b2a_rtt_min = pba->rtt_min;
        conns[count].b2a_rtt_max = pba->rtt_max;
        conns[count].b2a_rtt_sum = pba->rtt_sum;
        conns[count].b2a_rtt_sum2 = pba->rtt_sum2;
        conns[count].b2a_rtt_dupack = pba->rtt_dupack;
        conns[count].b2a_rtt_triple_dupack = pba->rtt_triple_dupack;
        conns[count].b2a_rtt_amback = pba->rtt_amback;
        conns[count].b2a_rtt_cumack = pba->rtt_cumack;
        conns[count].b2a_rtt_nosample = pba->rtt_nosample;
        conns[count].b2a_retr_max = pba->retr_max;
        conns[count].b2a_retr_min_tm = pba->retr_min_tm;
        conns[count].b2a_retr_max_tm = pba->retr_max_tm;
        conns[count].b2a_retr_tm_sum = pba->retr_tm_sum;
        conns[count].b2a_retr_tm_sum2 = pba->retr_tm_sum2;

        conns[count].b2a_initialwin_bytes = pba->initialwin_bytes;
        conns[count].b2a_initialwin_segs = pba->initialwin_segs;
        conns[count].b2a_idle_max = pba->idle_max;

        pba_last = (pba->reset_count > 0) ? pba->latest_seq : pba->fin;
        if (pba->syn_count > 0 && pba->fin_count > 0) {
            if (pba_last > pba->syn)
                stream_length_pba = (u_llong)pba_last - pba->syn - 1;
            else
                stream_length_pba = 0;
        } else {
            stream_length_pba = 0;
        }
        conns[count].b2a_stream_length = stream_length_pba;
        conns[count].b2a_missed_data = (stream_length_pba > pba->unique_bytes) ?
                                      (stream_length_pba - pba->unique_bytes) : 0;

        conns[count].b2a_data_xmit_time = elapsed(pba->first_data_time, pba->last_data_time) / 1000000.0;
        if (conns[count].elapsed_time > 0)
            conns[count].b2a_throughput = (double)pba->unique_bytes / (conns[count].elapsed_time / 1000000.0);
        else
            conns[count].b2a_throughput = 0.0;

        conns[count].b2a_window_scale = pba->window_scale;
        conns[count].b2a_f1323_ws = pba->f1323_ws;
        conns[count].b2a_f1323_ts = pba->f1323_ts;
        conns[count].b2a_fsack_req = pba->fsack_req;

        conns[count].filename = ptp->filename ? strdup(ptp->filename) : strdup("");
...
%s->%s: rexmitted FIN had diff. seqnum! (was %lu, now %lu, etime: %d sec)\n",
	...
	thisdir->data_pkts += 1;
	if (PUSH_SET(ptcp))
	    thisdir->data_pkts_push += 1;// initial window-packets a b
	thisdir->data_bytes += tcp_data_length;
        if (URGENT_SET(ptcp)) {     /* Checking if URGENT bit is set */
	    urg = TRUE; 
	    thisdir->urg_data_pkts += 1;
...
>>>>>>> 86f18337963adc76f3ea0534400b0dd84b3447de:tcptrace_python_api/trace.c
